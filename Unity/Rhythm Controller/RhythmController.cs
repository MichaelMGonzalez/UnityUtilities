...
public class RhythmController : MonoBehaviour {
    ...
    internal static RhythmController GetController()
    {
        return singleton;
    }
    ...
    // Part of the Dr. Eric Start Function
    void Setup()
    {
    ...
    protected void FireEvents(float t)
    {
            ...
            { // We know that we're in an appropriate measure to call methods on
                ...
                foreach (float timeKey in timeKeys)
                {
                    if (WithinErrorMargin(t % timeKey, timeKey))
                    {
                        ...
                        {
                            float ms = t / ( activeChannel == null ? 1 : activeChannel.pitch);
    ...
    protected void SetNoteLengths() { /** multiply by 1000 to convert to milliseconds*/
        wholeNote = 60f * 1000f / bpm * 4;
        halfNote = 60f * 1000f / bpm * 2;
        quarterNote = 60f * 1000f / bpm;
        eigthNote = 60f * 1000f / bpm / 2;
        sixteenthNote = 60f * 1000f / bpm / 4;
        tripleWholeNote = 60f * 1000f / bpm * 4 / 3;
        tripleHalfNote = 60f * 1000f / bpm * 2 / 3;
        tripleQuarterNote = 60f * 1000f / bpm / 3;
        tripleEigthNote = 60f * 1000f / bpm / 6;
        measureLength = quarterNote * timeSigUpper;
    }
